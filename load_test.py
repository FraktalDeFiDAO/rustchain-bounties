...
Tests the RustChain JSON-RPC API endpoints under load.
"""
...
class RustChainUser(HttpUser):
    ...
    @task(1)
    def eth_get_block_by_hash(self):
        payload = make_jsonrpc_request("eth_blockNumber", [])
        with self.client.post(RPC_URL, json=payload, catch_response=True) as response:
            if response.status_code != 200:
                return
...
def on_test_start(environment, **kwargs):
    ...
    print(
        f"Test endpoints: eth_blockNumber, eth_getBlockByNumber, eth_getBalance, eth_gasPrice, eth_chainId, eth_getTransactionByHash"
    )
